...
public class ThemeNodePlugin extends AbstractNodeAmPlugin {
	...
	protected Map<String, Iterable<? extends Class<? extends Node>>> getNodesByVersion() {
		return Collections.singletonMap(ThemeNodePlugin.currentVersion, 
				Collections.singletonList(ThemeNode.class));
	}
	...
	public void onInstall() throws PluginException {
		logger.info("Installing ThemeService service");
		pluginTools.installService(ThemeService.class);
		super.onInstall();
	}
	...
	public void onStartup(StartupType startupType) throws PluginException {
		logger.info("Starting ThemeService service");
		pluginTools.startService(ThemeService.class);
	...
	public void upgrade(String fromVersion) throws PluginException {
		...
		try {
			...
			if (sm.getServiceNames().contains("ThemeService")) {
				logger.info("removing old ThemeService version");
				sm.removeService("ThemeService", fromVersion);
			}
			logger.info("Installing new ThemeService service");
			pluginTools.installService(ThemeService.class);
		} catch (SSOException | SMSException e) {
			throw new PluginException(e.getMessage());
		}
		pluginTools.upgradeAuthNode(ThemeNode.class);
	...
	public String getPluginVersion() {
		return ThemeNodePlugin.currentVersion;
	}
}
